...
rem examples:
rem subscription_id=f5279601-9999-9999-9b11-9999999 // can find id for your subscription with 'az account show'
rem resource_group=AI102-Book // find or create a resource group with 'az group list'
rem location=eastus // find a location with 'az account list-locations'
...
call az storage container create --account-name ai102str!unique_id! --name hotels --public-access blob --auth-mode key --account-key %AZURE_STORAGE_KEY% --output none
call az storage blob upload-batch -d hotels -s data --account-name ai102str!unique_id! --auth-mode key --account-key %AZURE_STORAGE_KEY%  --output none
